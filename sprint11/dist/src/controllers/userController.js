...
userController.post('/users', async (req, res, next) => {
    try {
        const user = await userService_js_1.default.createUser(req.body);
        const accessToken = userService_js_1.default.createToken(user.id, 'access');
        const refreshToken = userService_js_1.default.createToken(user.id, 'refresh');
        await userService_js_1.default.updateUser(user.id, { refreshToken });
        setRefreshTokenCookie(res, refreshToken);
        res.status(httpStatus_js_1.default.CREATED).json({ accessToken, user });
    }
    catch (error) {
        next(error);
    }
...
userController.post('/login', async (req, res, next) => {
    ...
    try {
        const user = await userService_js_1.default.getUser(email, password);
        const accessToken = userService_js_1.default.createToken(user.id, 'access');
        const refreshToken = userService_js_1.default.createToken(user.id, 'refresh');
        await userService_js_1.default.updateUser(user.id, { refreshToken });
        setRefreshTokenCookie(res, refreshToken);
...
userController.get('/auth/google', passport_js_1.default.authenticate('google', { scope: ['profile', 'email'] }));
userController.get('/auth/google/callback', passport_js_1.default.authenticate('google'), (req, res, next) => {
    try {
        const { id: userId } = req.user;
        const accessToken = userService_js_1.default.createToken(userId, 'access');
        const refreshToken = userService_js_1.default.createToken(userId, 'refresh');
        setRefreshTokenCookie(res, refreshToken);
...
userController.post(RENEW_TOKEN_PATH, passport_js_1.default.authenticate('refresh-token', { session: false }), async (req, res, next) => {
    try {
        const { refreshToken } = req.cookies;
        const { id: userId } = req.user;
        const { accessToken, newRefreshToken } = await userService_js_1.default.refreshToken(userId, refreshToken);
        await userService_js_1.default.updateUser(userId, { refreshToken: newRefreshToken }); // 추가
